...
class MainViewController: UIViewController, SideMenuViewControllerDelegate {
    ...
    private var sideMenuViewController : SidebarVC! //4
    ...
    override func viewDidLoad() {
        ...
        self.setNavBarAppearance(tintColor: .white, barColor: UIColor.purple)

        // Shadow Background View
   /*     self.sideMenuShadowView = UIView(frame: self.view.bounds)
        self.sideMenuShadowView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        self.sideMenuShadowView.backgroundColor = #colorLiteral(red: 0.1930268705, green: 0.1930268705, blue: 0.1930268705, alpha: 1)
    ...
    */
       ...
       // self.sideMenuViewController.delegate = self
        self.sideMenuViewController.sideBarDelegate? = self
  ...
  /*      let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(handlePanGesture))
    ...
    func selectedCell(group: TableSection, rowSelected: Int) {
        switch  (group){
            case .DashBoardGroup :
                if (rowSelected == 0) {
                    // Home
                    self.showViewController(viewController: UINavigationController(rootViewController: HomeVC()))
                }
                if (rowSelected == 2) {
                    // Home
                    self.showViewController(viewController: UINavigationController(rootViewController: RecipeVC()))
                   // self.navigationController?.pushViewController(RecipeVC(), animated: true)
                }
            case .ShopGroup:
                if rowSelected == 0 {
                    self.showViewController(viewController: UINavigationController(rootViewController: ShopVC()))
                } else if rowSelected == 1 {
                    self.showViewController(viewController: UINavigationController(rootViewController: VideoVC()))
                }
            case .FaqGroup :
                if rowSelected == 2 {
                    self.showViewController(viewController: UINavigationController(rootViewController: FaqVC()))
                }
            case .LogoutGroup:
                break
            }
    ...
    func showViewController(viewController:UINavigationController)->(){
